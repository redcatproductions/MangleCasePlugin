...
public class ToUpperCaseActionDelegate extends ActionDelegate implements
	...
	public void run(IAction action) {
		...
		if (editorPart instanceof ITextEditor) {
			...
			convertToUpperCase(selection, editor);
		}
	}

	private void convertToUpperCase(final ITextSelection selection,
			final ITextEditor editor) {
		IDocumentProvider provider = editor.getDocumentProvider();
		IDocument document = provider.getDocument(editor.getEditorInput());
		try {
			document.replace(selection.getOffset(), selection.getLength(),
					StringUtils.upperCase(selection.getText()));
