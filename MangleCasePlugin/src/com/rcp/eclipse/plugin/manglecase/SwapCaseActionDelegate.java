...
public class SwapCaseActionDelegate extends ActionDelegate implements
	...
	public void run(IAction action) {
		...
		if (editorPart instanceof ITextEditor) {
			...
			swapCase(selection, editor);
		}
	}

	private void swapCase(final ITextSelection selection,
			final ITextEditor editor) {
		IDocumentProvider provider = editor.getDocumentProvider();
		IDocument document = provider.getDocument(editor.getEditorInput());		
		try {
			document.replace(selection.getOffset(), selection.getLength(),
					StringUtils.swapCase(selection.getText()));
